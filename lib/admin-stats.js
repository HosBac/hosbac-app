...
async function buildAdminStats() {
  ...
  // Lecture exclusive sur Turso SQL
  ...
  for (const data of userDocs) {
    const createdAt = toDate(data.created_at || data.createdAt || data.date);
  ...
  for (const data of examDocs) {
    const createdAt = toDate(data.created_at || data.createdAt || data.date_ajout || data.date);
    const status = String(data.status || 'approved');
    const downloads = Number(data.download_count || data.downloadCount || data.downloads || data.nb_downloads || 0) || 0;
    const views = Number(data.view_count || data.viewCount || data.views || data.nb_views || 0) || 0;
  ...
  try {
    ...
    if (err.message && (err.message.includes('has no column') || err.message.includes('no column named'))) {
      await turso.execute("DROP TABLE IF EXISTS admin_stats");
      await turso.execute(`
