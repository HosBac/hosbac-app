...
async function buildAdminStats() {
  ...
  // Stockage du rapport compilé dans Turso avec gestion d'erreur de structure
  try {
    await turso.execute(`
    ...
    // Si la table avait une mauvaise structure (colonne manquante), on la réinitialise
    if (err.message && (err.message.includes('has no column') || err.message.includes('no column named'))) {
      console.warn('[ADMIN STATS] Réinitialisation de la table admin_stats obsolète...');
